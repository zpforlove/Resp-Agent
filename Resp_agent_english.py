...
def run_diagnoser(user_prompt: str) -> str:
    ...
    metadata_csv = _clean_path(metadata_csv_match.group(1)) if metadata_csv_match else None

    # Fallback: try to infer from raw paths in the prompt
    if not audio_dir:
        m = re.search(r"(\.{0,2}/[^\s,]+/audio[^\s,]*)", user_prompt, flags=re.IGNORECASE)
        if m:
            audio_dir = _clean_path(m.group(1))
    if not output_dir:
        m = re.search(r"(\.{0,2}/[^\s,]*(?:output)[^\s,]*)", user_prompt, flags=re.IGNORECASE)
        if m:
    ...
    if not metadata_csv:
        ...
        messages.append(f"No metadata_csv found in prompt, using default: {metadata_csv}")
        messages.append(
    ...
    command = [
        "python", "-u", "./Diagnoser/diagnoser_pipeline.py",
        "--audio_dir", audio_dir,
            ...
            if result_path and os.path.exists(result_path):
                try:
                    ...
                    if mismatches.empty:
                        ...
                        error_counts = mismatches.groupby(
...
def run_generator(user_prompt: str) -> str:
    ...
    disease_match = re.search(dis_pat1, user_prompt, flags=re.IGNORECASE) or \
                    re.search(dis_pat2, user_prompt, flags=re.IGNORECASE)
    ...
    if not out_dir:
        m = re.search(r"(\.{0,2}/[^\s,]*(?:output|out)[^\s,]*)", user_prompt, flags=re.IGNORECASE)
        if m:
    ...
    command = [
        "python", "-u", "./Generator/generator_pipeline.py",
        "--ref_audio", ref_audio,
        "--disease", disease,
        "--out_dir", out_dir
    ]
    ...
    while True:
        try:
            ...
            while True:
                ...
                call_diag_match = re.search(r"\[Call:Diagnoser\](.*)", assistant_response, re.DOTALL)
                call_gen_match = re.search(r"\[Call:Generator\](.*)", assistant_response, re.DOTALL)
                final_answer_match = re.search(r"\[Final Answer\](.*)", assistant_response, re.DOTALL)
