...
def find_best_checkpoint(ckpt_dir: str) -> str:
    ...
    if best_path is None:
        raise RuntimeError(
            "Could not parse loss value from filename, please check the saving naming convention."
        )
    print(
        f"Found best checkpoint: {os.path.basename(best_path)} (loss={best_loss:.4f})\nPath: {best_path}"
    )
    return best_path
...
def load_jsonl_metadata(jsonl_path: str):
    ...
    if not meta:
        raise RuntimeError(
            f"No valid entries (audio_filename+disease+description) were parsed from {jsonl_path}."
        )
...
class LFAudioDataset(Dataset):
    def __init__(
        ...
        if not self.records:
            raise RuntimeError(
                "No files in the test set could be matched in the metadata (with disease+description)."
            )
        print(
            f"Number of test samples: {len(self.records)}, skipped (no label/description): {skipped}"
        )
...
class LongformerWithBEATsInfer:
    ...
    def prepare_batch(self, waveforms: torch.Tensor, descriptions: list):
        ...
        with torch.no_grad():
            feats, _ = self.beats.extract_features(
                waveforms.to(self.device), padding_mask=None
            )
        ...
        for i in range(B):
            desc = (
                descriptions[i]
                if isinstance(descriptions[i], str)
                else str(descriptions[i])
            )
            desc_ids = self.tokenizer.encode(
                desc, add_special_tokens=False, truncation=True, max_length=desc_budget
            )
            seq = (
                [cls_id, self.desc_tok_id] + desc_ids + self.audio_embed_ids + [sep_id]
            )
            if len(seq) > self.max_seq_len:
                ...
                if over > 0 and len(desc_ids) > 0:
                    ...
                    seq = (
                        [cls_id, self.desc_tok_id]
                        + desc_ids
                        + self.audio_embed_ids
                        + [sep_id]
                    )
                if len(seq) > self.max_seq_len:
        ...
        # attention mask
        attn_mask_list = [
            [1] * len(seq) + [0] * (max_len - len(seq)) for seq in batch_ids
        ]
...
# ============================
def evaluate(
    model_wrap: LongformerWithBEATsInfer,
...
def main():
    parser = argparse.ArgumentParser(
        description="Longformer Test (Inference & Evaluation)"
    )
    parser.add_argument(
    ...
    # All paths sourced from config
    test_jsonl = cfg["data"]["metadata_jsonl"]
    test_wav_dir = cfg["data"]["test_root"]
    ckpt_dir = os.path.join(cfg["paths"]["checkpoint_dir"], "longformer")
    ...
    if not idx_to_disease:
        raise RuntimeError(
            "Missing label_mapping (idx->disease) in the checkpoint's client_state."
        )
    ...
    batch_size = int(
        cfg.get("hyperparameters", {}).get("longformer", {}).get("eval_batch_size", 4)
    )
    loader = DataLoader(
        dataset,
    ...
    test_loss, test_acc, test_f1, report = evaluate(
        infer, loader, criterion, device, idx_to_disease
    )
